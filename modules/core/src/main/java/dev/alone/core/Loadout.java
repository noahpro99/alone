...
public final class Loadout {
    ...
    /** The welcome on first join: your biome, and that you may bring two things. */
    ...
    private static int list(CommandSourceStack source) throws CommandSyntaxException {
        ...
        source.sendSuccess(() -> Component.literal(
            "You woke in " + biomeName(player) + ". You may still pack " + left + " item"
                + (left == 1 ? "" : "s") + " — choose against the land you're in:"), false);
        for (Choice c : CHOICES) {
    ...
    private static int pick(CommandSourceStack source, String key) throws CommandSyntaxException {
        ...
        source.sendSuccess(() -> Component.literal("Packed: " + packed.label() + "."
            + (left > 0 ? " " + left + " pick left." : " Your kit is set — good luck.")), false);
        return Command.SINGLE_SUCCESS;
    }
