...
- **Condition panel replaces hearts (§1.5)** — DONE (MVP): vanilla hearts hidden
  (`HudElementRegistry.removeElement(HEALTH_BAR)`); HUD now shows a **vitality bar** (from health,
  reskinned) + the paper-doll injury readout. Remaining depth: divorce death from the raw damage
  number (translate all hits into conditions that drain vitality) and slow vitality regen to days.
