...
- [x] **Eating takes real time (§1.1)** — vanilla eats a whole meal in 1.6s; now **3× longer** (a snack ~5s), relative differences preserved, so you can't scarf food mid-scramble (pairs with no-eating-while-sprinting). Drinks/potions untouched. (`FoodUseDurationMixin`.)
- [x] **Craft-time tuning** — stations (chest/furnace) bumped 30s → **60s** (real joinery/masonry); metal tools/armor keep moderate grid times since the **forge system adds the metalwork time on top** (no double-counting). (`CraftingTime`.)
